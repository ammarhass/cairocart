        ...
        if res.account_type.startswith("asset"):
            for record in self.env['account.financial.report'].search(
                ...
                if record.get_metadata()[0].get(
                        'xmlid') == 'dynamic_accounts_report.account_financial_report_assets0':
            ...
            for record in self.env['account.financial.report'].search(
                    [('type', '=', 'account_type')]):
                if record.get_metadata()[0].get(
                        'xmlid') == 'dynamic_accounts_report.account_financial_report_liability0':
                    record.write({"account_ids": [(4, res.id)]})
        elif res.account_type in ['expense', 'expense_depreciation']:
            for record in self.env['account.financial.report'].search(
                    [('type', '=', 'account_type')]):
                if record.get_metadata()[0].get(
                        'xmlid') == 'dynamic_accounts_report.account_financial_report_expense0':
                    record.write({"account_ids": [(4, res.id)]})
        elif res.account_type == "expense_direct_cost":
            for record in self.env['account.financial.report'].search(
                    [('type', '=', 'account_type')]):
                if record.get_metadata()[0].get(
                        'xmlid') == 'dynamic_accounts_report.financial_report_cost_of_revenue':
                    record.write({"account_ids": [(4, res.id)]})
        elif res.account_type in ['income', 'equity_unaffected']:
            for record in self.env['account.financial.report'].search(
                    [('type', '=', 'account_type')]):
                if record.get_metadata()[0].get(
                        'xmlid') == 'dynamic_accounts_report.account_financial_report_operating_income0':
                    record.write({"account_ids": [(4, res.id)]})
        elif res.account_type == 'income_other':
            for record in self.env['account.financial.report'].search(
                    [('type', '=', 'account_type')]):
                if record.get_metadata()[0].get(
                        'xmlid') == 'dynamic_accounts_report.account_financial_report_other_income0':
                    record.write({"account_ids": [(4, res.id)]})
        return res
